...
export default function Meals() {
  ...
  const proceedToCart = async () => {
    ...
    try {
      ...
      const fullIngredients = selectedMeals
        .flatMap((meal) => meal.ingredients || [])
        .filter(Boolean);

      if (fullIngredients.length === 0) {
        throw new Error("No ingredients found for the selected meals.");
      }
          ...
          return (
            ...
            >
              <div className="flex justify-between items-start mb-6">
                ...
                <div className={`w-9 h-9 rounded-md border-brutal border-black flex items-center justify-center transition-all shadow-neo-sm ${isSelected ? "bg-[#E79B48]" : "bg-white"}`}>
              ...
              <h4 className="text-sm font-bold text-[#164E40] mb-3 uppercase tracking-wider opacity-80">Recipe Ingredients</h4>
      ...
      <div className="mt-16 flex flex-col items-center gap-4">
        ...
        >
          {loading ? (
            ...
            `Go to Cart (${selectedIndices.length} dish${selectedIndices.length !== 1 ? "es" : ""} selected)`
          )}
        </button>
      </div>
    </section>
